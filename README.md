...
- **Creating and editing** is one column, top to bottom: title, description, the content, "How to use", then a **Details** accordion (Where it works, Built for, Visibility, Editors) that opens one row at a time and shows its current value in the header. New items start with sensible defaults (the first catalog app, the first team, public) so the required fields are only a title and the content; a `REQUIRED` chip marks them, each label carries an example, and the footer hint says what's still missing until it reads "Everything needed is filled in." A **new skill** first asks how it arrives: *Upload a .skill file* (a drop zone that unpacks the archive, fills the title and description from SKILL.md and puts SKILL.md first), *Write it here* (starts from the SKILL.md template), or *Just link it* (a link row for a Claude project, custom GPT, or Town agent). A chip row lets you switch route at any time without losing what you've entered; blank link rows are dropped on the way out, and "Add a link as well" / "Add files as well" combines the two. Editing an existing item skips the chooser. Forking shows the source item and a "What did you change" note.
- **Links on prompts and skills.** Any item can carry links to where it lives or is saved (a Granola recipe, a Claude project, a custom GPT, a Town agent). On a prompt the editor offers "Add a link to where it's saved" under the prompt text; the detail page then shows an "Open it where it's saved" card above the prompt so people can open the recipe instead of pasting the text each time. Skills use the same links block ("Open it where it lives"), and a skill may be link-only. Up to 20 http(s) links per item, stored in `prompts.links` and snapshotted in version history.
- **Recommended and required models.** Each app on a prompt or skill can name the model it works best with ("Opus 4.6", "GPT-5 Thinking") and whether that model is *recommended* or *required*. The editor's "Where it works" row is a list of app rows: tick an app to expand it, pick surfaces, type the model (the placeholder comes from the catalog's per-app model hint, editable in /admin), and choose Recommended or Required once a model is set. The detail page shows a **Model** card per app in the side column with a Required or Recommended badge; cards show a small badge with the first required model. Stored on `prompt_apps.model` / `model_required`; the Slack assistant sees models when choosing and says "requires X" on its cards.
